...
;(function (root, factory) {
    ...
    jedfn.eachHms = function (opts, boxCell) {
        ...
        $.each(new Array(ranges ? 1 : 2), function (m) {
            ...
            if (multiPane == false && ranges) {
                timeList.css({
                    "padding-left": timeList.outerWidth() / 2 + 12,
                    "padding-right": timeList.outerWidth() / 2 + 12
                })
            }
        });
        //计算当前时分秒的位置
    ...
    jedfn.clickBtn = function (opts, boxCell) {
        ...
        boxCell.on("click", ".clear", function (ev) {
            ...
            if (isShow) {
                ...
                if (gtval != "") {
                    ...
                    if ($.isFunction(opts.clearfun) || opts.clearfun != null) opts.clearfun({
                        elem: elemCell,
                        val: clearVal
                    });
                }
        ...
        boxCell.on("click", ".today", function () {
            ...
            that.dateClose();
            if ($.isFunction(opts.okfun) || opts.okfun != null) opts.okfun({
                elem: elemCell,
                val: thisdate,
                date: objVal
            });
